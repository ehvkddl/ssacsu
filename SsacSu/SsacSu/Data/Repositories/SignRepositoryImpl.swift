...
extension SignRepositoryImpl: SignRepository {
    ...
    func checkEmailValidation(email: String) -> Single<Result<Bool, SsacsuError>> {
        ...
        return networkService.processResponse(api: SsacsuAPI.checkEmailValidation(email: request), responseType: nil)
    }
    
    func join(join: Join) -> Single<Result<JoinResponseDTO, SsacsuError>> {
        let request = join.toRequest()
        return networkService.processResponse(api: SsacsuAPI.join(join: request), responseType: JoinResponseDTO.self)
    }
    
    func appleLogin(with login: AppleLogin) -> Single<Result<AppleLoginResponseDTO, SsacsuError>> {
        let request = login.toRequest()
        
        return networkService.processResponse(api: SsacsuAPI.appleLogin(login: request), responseType: AppleLoginResponseDTO.self)
    }
    
    func kakaoLogin(with login: KakaoLogin) -> Single<Result<KakaoLoginResponseDTO, SsacsuError>> {
        let request = login.toRequest()
        
        return networkService.processResponse(api: SsacsuAPI.kakaoLogin(login: request), responseType: KakaoLoginResponseDTO.self)
    }
    
    func login(with login: Login) -> Single<Result<LoginResponseDTO, SsacsuError>> {
        let request = login.toRequest()
        
        return networkService.processResponse(api: SsacsuAPI.login(login: request), responseType: LoginResponseDTO.self)
    }
